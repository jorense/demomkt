...
<body>
	<div id="wrap">
        ...
        --><div id="content_wrap">
            <h1>Edit the content for the Career section</h1>
		
            <div id="divider">&nbsp;</div>
            <h3 id="navigation_menu">Career Content</h3>
            <p>The content pages of the Career section</p>
            <?php 
				// Table variables
				$db_table = 'content_index';
				$input_form_id = '53';
				$reference_column = 'section';
				$reference_id = 'career_content';
				// Bulkremove Variables
				$br_form = 'career_content_bulkremove';
				$br_button = 'career_content_button';
				$br_checkbox = 'career_content_check';
			?>
                    	...
                    	<form class="<?php echo $br_form; ?>" method="post" action="bulkremove.php">
							...
							<?php
								...
								while($row = mysqli_fetch_array($result)) {
									...
									// Absolute Path (EN)
									if($active_en=='Yes') {
										$ab_semantic_url_en = $real_root_directory . 'en/career/' . $semantic_url;
									...
									// Absolute Path (SC)
									if($active_sc=='Yes') {
										$ab_semantic_url_sc = $real_root_directory . 'sc/career/' . $semantic_url;
									...
									// Relative Path (EN)
									if($active_en=='Yes') {
										$rel_semantic_url_en = 'en/career/' . $semantic_url;
									...
									// Relative Path (SC)
									if($active_sc=='Yes') {
										$rel_semantic_url_sc = 'sc/career/' . $semantic_url;
