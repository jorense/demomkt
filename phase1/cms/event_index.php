...
<body>
	<div id="wrap">
        ...
        --><div id="content_wrap">
            <h1>Edit the content for the Event section</h1>
		
            <div id="divider">&nbsp;</div>
            <h3 id="navigation_menu">Event Index</h3>
            <p>The content of the Event index</p>
            <?php 
				// Table variables
				$db_table = 'content_index';
				$input_form_id = '54';
				$reference_column = 'section';
				$reference_id = 'event_index';
                    ...
                    <tbody>
						<?php
							...
							while($row = mysqli_fetch_array($result)) {
								...
								// Absolute Path (EN)
								if($active_en=='Yes') {
									$ab_semantic_url_en = $real_root_directory . 'en/events/';
								...
								// Absolute Path (SC)
								if($active_sc=='Yes') {
									$ab_semantic_url_sc = $real_root_directory . 'sc/events/';
            ...
            <div id="divider">&nbsp;</div>
            <h3 id="navigation_menu">Event Content</h3>
            <p>The content pages of the Event section</p>
            <?php 
				// Table variables
				$db_table = 'content_index';
				$input_form_id = '55';
				$reference_column = 'section';
				$reference_id = 'event_content';
				// Bulkremove Variables
				$br_form = 'event_content_bulkremove';
				$br_button = 'event_content_button';
				$br_checkbox = 'event_content_check';
			?>
                ...
                <table id="table_dump">
                    <thead>
                        <tr>
							...
							<th scope="col" class="sort">Registration Received <a href="javascript:void(0)"><i class="fa fa-sort"></i></a></th>
							<th scope="col" class="sort">Absolute Path (EN) <a href="javascript:void(0)"><i class="fa fa-sort"></i></a></th>
							<th scope="col" class="sort">Absolute Path (SC) <a href="javascript:void(0)"><i class="fa fa-sort"></i></a></th>
							<th scope="col" class="sort">Relative Path (EN) <a href="javascript:void(0)"><i class="fa fa-sort"></i></a></th>
							<th scope="col" class="sort">Relative Path (SC) <a href="javascript:void(0)"><i class="fa fa-sort"></i></a></th>
                    	...
                    	<form class="<?php echo $br_form; ?>" method="post" action="bulkremove.php">
							...
							<?php
								...
								while($row = mysqli_fetch_array($result)) {
									...
									");
									$registration_received = mysqli_num_rows($rsvp_result);
									
									// Absolute Path (EN)
									if($active_en=='Yes') {
										$ab_semantic_url_en = $real_root_directory . 'en/event/' . $semantic_url;
										$ab_semantic_url_en = '<a href="' . $ab_semantic_url_en . '" target="_blank">' . $ab_semantic_url_en . '</a>';	
									}
									else {
										$ab_semantic_url_en = '';
									...
									// Absolute Path (SC)
									if($active_sc=='Yes') {
										$ab_semantic_url_sc = $real_root_directory . 'sc/event/' . $semantic_url;
										$ab_semantic_url_sc = '<a href="' . $ab_semantic_url_sc . '" target="_blank">' . $ab_semantic_url_sc . '</a>';
									}
									else {
										$ab_semantic_url_sc = '';
									...
									// Relative Path (EN)
									if($active_en=='Yes') {
										$rel_semantic_url_en = 'en/event/' . $semantic_url;
										$rel_semantic_url_en = '<a href="../../' . $rel_semantic_url_en . '" target="_blank">' . $rel_semantic_url_en . '</a>';	
									}
									else {
										$rel_semantic_url_en = '';
									...
									// Relative Path (SC)
									if($active_sc=='Yes') {
										$rel_semantic_url_sc = 'sc/event/' . $semantic_url;
										$rel_semantic_url_sc = '<a href="../../' . $rel_semantic_url_sc . '" target="_blank">' . $rel_semantic_url_sc . '</a>';
									}
									else {
										$rel_semantic_url_sc = '';
									...
									echo '
										...
										<tr class="' . $bkg . '">
											...
											<td>' . $meta_description_en . '</td>
											<td>' . $meta_description_sc . '</td>
											<td>' . $parameter_data . '</td>
											<td>' . $registration_received . '</td>
											<td>' . $ab_semantic_url_en . '</td>
											<td>' . $ab_semantic_url_sc . '</td>
											<td>' . $rel_semantic_url_en . '</td>
											<td>' . $rel_semantic_url_sc . '</td>
											...
											<td>' . $remark . '</td>
											<td><a href="event_rsvp_download.php?parameter_data=' . $parameter_data . '" target="download">Download RSVP (.xls)</a></td>
											<td><a href="form.php?task=edit&form_id=' . $input_form_id . '&identifier=' . $identifier . '&reference_id=' . $reference_id . '&return_link=' . $url .'">Edit</a></td>
										</tr>
									';
									$i++;
            ...
            <div id="divider">&nbsp;</div>
            <h3 id="navigation_menu">Event RSVP</h3>
            <p>The visitors who have signed up for an event.</p>
            <?php 
				// Table variables
				$db_table = 'event_rsvp';
				$input_form_id = '';
				...
				$br_checkbox = '';
			?>
            <a href="event_rsvp_download.php?parameter_data=all" target="download">
                <div class="button">
                    Download .xls
                </div>
            </a>
